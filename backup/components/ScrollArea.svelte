...
<script lang="ts">
	...
	let showX = false;
	let { debounce: debounceShowX, destroy: destroyShowX } = debouncing(scrollHideDelay);

	let showY = false;
	let { debounce: debounceShowY, destroy: destroyShowY } = debouncing(scrollHideDelay);
	...
	const handleScroll = () => {
		...
		thumbXPosition =
			(scrollable.scrollLeft / ((scrollable as any).scrollLeftMax + scrollable.clientWidth)) *
				100 ?? 0;
		thumbYPosition =
			(scrollable.scrollTop / ((scrollable as any).scrollTopMax + scrollable.clientHeight)) * 100 ??
			0;

		if (finalTypeX === 'scroll' || finalTypeX === 'hover' || finalTypeX === 'auto') {
			showX = true;
			debounceShowX(() => (showX = false));
		}
		if (finalTypeY === 'scroll' || finalTypeY === 'hover' || finalTypeY === 'auto') {
			showY = true;
	...
	const handleMouseDownY = (event: MouseEvent) => {
		...
		const boundingTarget = (event.target as HTMLElement).getBoundingClientRect();
...
<svelte:element this={tag} {...$$restProps} class={buildClass('scroll-area', $$restProps.class)}>
	...
	{#if showX}
		<div
			class={buildClass('scroll-area__scrollbar', 'scroll-area__scrollbar-horizontal')}
			style:--scrollbar-width={`${scrollbarSize}px`}
	...
	{#if showY}
		<div
			class={buildClass('scroll-area__scrollbar', 'scroll-area__scrollbar-vertical')}
			style:--scrollbar-width={`${scrollbarSize}px`}
			style:--scrollbar-thumb-width={`${thumbSize}px`}
